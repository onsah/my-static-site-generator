...
The last problem is that when the assumptions about a particular is changed, it's not propagated downwards and you end up with many new bugs that you are not aware. For example, you check a variable is valid, then perform some operation on it. But later if you add a code that mutates that variable in between, you will probably forget to check it after the mutation. It is easy to do this mistake when there are many variables involved or there are deep call chains.
