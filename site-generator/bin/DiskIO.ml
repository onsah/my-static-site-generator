...
let write_all path ~content = Out_channel.write_all (Path.to_string path) ~data:content
...
let read_all path =
  ...
  | Unknown ->
    failwith (sprintf "Illegal Argument: '%s' doesn't exist." (Path.to_string path))
;;

let list path =
  match get_type path with
  | Directory ->
    List.map (Sys_unix.readdir (Path.to_string path) |> List.of_array) ~f:Path.from
  | _ ->
    failwith (sprintf "Illegal Argument: '%s' is not a directory." (Path.to_string path))
;;
